...
export class TechnologiesService {
  ...
  async create(request: Request, createTechnologiesDto: CreateTechnologiesDto): Promise<TechnologiesEntity> {
    try {
      ...
      const category = await this.categoriesService.findOne(createTechnologiesDto.fk_category);
      if (!category) {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology not created because category  ${category} does not exists` } as CreateLogDto;
      ...
      const ring = await this.categoriesService.findOne(createTechnologiesDto.fk_ring);
      if(!ring) {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology not created because ring  ${ring} does not exists` } as CreateLogDto;
        await this.logService.create(createLogDto);
      ...
      if(await this.authService.isAdmin(request)) {
        ...
        dto.published = false;
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.INFO, description: `Technology with id ${dto._id} created` } as CreateLogDto;
        await this.logService.create(createLogDto);
        return dto.save();
      }
    } catch (error) {
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.INFO, description: `Technology could not be created ${ error } ` } as CreateLogDto;
      await this.logService.create(createLogDto);
  ...
  async findOne(request: Request, _id: string): Promise<TechnologiesEntity> {
    ...
    try{
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.DEBUG, description: `Technology found with id ${ _id } ` } as CreateLogDto;
      await this.logService.create(createLogDto);
      return technology;
    } catch {
  ...
  async findAll(request: Request): Promise<TechnologiesEntity[]> {
    try {
      if (await this.authService.isAdmin(request)) {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.DEBUG, description: `Technologies found for admin` } as CreateLogDto;
        await this.logService.create(createLogDto);
        return this.technologiesModel.find().exec();
      } else {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technologies found for user` } as CreateLogDto;
        await this.logService.create(createLogDto);
        return this.technologiesModel.find({ published: true }).exec();
      }
    } catch (error) {
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `No technology found   ${error}` } as CreateLogDto;
      await this.logService.create(createLogDto);
      throw new NotFoundException('No technologies found');
    }
  }
  ...
  async update(request: Request, _id: string, updateTechnology: UpdateTechnologiesDto): Promise<TechnologiesEntity> {
    ...
    const technology = await this.technologiesModel.findById(_id).exec();
    if (!technology) {
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology with _id ${_id} not found during technology update` } as CreateLogDto;
      await this.logService.create(createLogDto);
    ...
    if(await this.authService.isAdmin(request)) {
      try {
        ...
        if (technology.fk_ring === undefined || technology.fk_ring === '' || technology.fk_category === undefined || technology.fk_category === '') {
          ...
          const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology with _id ${_id} updated but not published` } as CreateLogDto;
          await this.logService.create(createLogDto);
        } else {
          const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology with _id ${_id} updated and published` } as CreateLogDto;
          await this.logService.create(createLogDto);
        }
        await this.technologiesModel.findByIdAndUpdate(_id, technology, { new: true }).exec();
        return technology;
      } catch (error) {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology with _id ${_id} could not be updated` } as CreateLogDto;
        await this.logService.create(createLogDto);
        throw new Error(`Technology with _id ${_id} could not be updated`);
      }
    } else {
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology not updated due to insufficient privileges` } as CreateLogDto;
      await this.logService.create(createLogDto);
      throw new Error(`Technology with _id ${_id} could not be updated`);
    }
  }
  ...
  async delete(request: Request, _id: string): Promise<TechnologiesEntity> {
    const technology = await this.technologiesModel.findById(_id).exec();
    if (!technology) {
      const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.ERROR, description: `Technology with _id ${_id} not found during technology deletion` } as CreateLogDto;
      await this.logService.create(createLogDto);
      throw new NotFoundException(`Technology with _id ${_id} not found`);
    }
    if(await this.authService.isAdmin(request)) {
      try {
        ...
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.INFO, description: `Technology with _id ${_id} deleted` } as CreateLogDto;
        await this.logService.create(createLogDto);
        return technology;
      } catch (error) {
        const createLogDto = { service: ServiceName.TECHNOLOGIES, severity: Severity.INFO, description: `Technology with _id ${_id} could not be deleted` } as CreateLogDto;
        await this.logService.create(createLogDto);
