...
export class LogService {
    ...
    async create(request, createLogDto: CreateLogDto): Promise<LogEntity> {
        ...
        try {
            ...
            const createLogDto = {
                ...
                description: `No logs found ${error}`,
                public: false
            } as CreateLogDto;
    ...
    async findAll(request: Request): Promise<LogEntity[]> {
        try {
            ...
            await this.create(request, createLogDto);
            return await this.logEntityModel.find().sort({creationDate: -1}).limit(100).exec();
        } catch (error) {
            const createLogDto = {
                service: ServiceName.LOG,
                severity: Severity.ERROR,
    ...
    async findPublicLogs(request: Request): Promise<LogEntity[]> {
        try {
            const createLogDto = {
                service: ServiceName.LOG,
                severity: Severity.INFO,
                description: `Get all puglic logs`,
                public: false
            } as CreateLogDto;
            await this.create(request, createLogDto);
            return await this.logEntityModel.find({public: true}).sort({creationDate: -1}).limit(100).exec();
        } catch (error) {
            const createLogDto = {
                service: ServiceName.LOG,
                severity: Severity.ERROR,
                description: `No public logs found ${error}`,
                public: false
            } as CreateLogDto;
            await this.create(request, createLogDto);
